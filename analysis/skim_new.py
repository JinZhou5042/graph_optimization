...
class Skimmer:
    ...
    @dask.delayed
    def write_partition(self, partitioned_events, outpath):
        ak.to_parquet(partitioned_events, outpath)
        return outpath

    @dask.delayed
    def merge_partitions(self, partition_paths, outpath, final=False):
        events = ak.from_parquet(partition_paths)
        ...
        if not final: return outpath

    # def write_and_merge(self, fileset, partition_merge_size=4,):
    ...
    #         # First write all partitions
    #         npartitions = events.npartitions
    #         written_partitions = []
    #         for i in range(npartitions):
    #             if npartitions == 1:
    #                 tag = final_tag
    #                 file_path = f"{self.output_dir}/{tag}.parquet"
    #             else:
    #                 tag = f"{final_tag}_part{i}"
    #                 file_path = f"{self.tmp_dir}/{tag}.parquet"

    #             future_file_path = self.write_partition(
    #                 events.partitions[i],
    #                 file_path,
    #                 )
    ...
    #             to_merge = [written_partitions.pop(0) for _ in range(partition_merge_size) if len(written_partitions)>0]
    ...
    def write_partitions(self, fileset, optimize_graph=False):
        ...
        for dataset_name, info in fileset.items():
            ...
            delayed  = []
            #merge_size = int(events.npartitions/2)
            #merge_size = events.npartitions
            print('Nmber of Partitions:', events.npartitions)
            merge_size = 7
            import math
            import os
            ...
            #dask.visualize(graph, filename="three_redux.pdf")
            #exit(1)
            if dsets >= 1:
                break
        return graph
    ...
    def connect(self, delayed, path):
        return path

    #def generate_reduction(self, events):
    #    return events.to_delayed(optimize_graph=False)

    #def generate_reduction(self, events):
    #    return events.to_delayed(optimize_graph=True)

    #def generate_reduction(self, events, merge_size=2):
    #    merged_partitions = []
    #    start = 0
    #    stop = merge_size
    #    print(events)
    #    print(dir(events.partitions[start:stop]))
    #    while(events.partitions[start:stop].npartitions):
    #        merged_partitions.append(dask.delayed(self.merge_partitions_ak)(events.partitions[start:stop]))
    #        start += merge_size
    #        stop += merge_size
    #    while(len(merged_partitions) > 1):
    #        to_merge = [merged_partitions.pop() for i in range(merge_size) if merged_partitions]
    #        x = dask.delayed(self.merge_partitions_ak)(*to_merge)
    #        merged_partitions.append(x)
    #    
    #    return merged_partitions[0]

    def merge_partitions_ak(self, *events):
        return ak.concatenate(events)

    def write_partitions_ak(self, events):
        pass
    ...
    def skim(
        ...
        **test_args
        ...
        if use_dask_vine:
            ...
            m = DaskVine(
                manager_port,
                name=f"{os.environ['USER']}-skim-manager",
                run_info_path=run_info_path,
                )
            m.tune('wait-for-workers', 10)
        ...
        # # Write and Merge
        # t2 = time.time()
        # graph = self.write_and_merge(filesets, partition_merge_size=partition_merge_size)
        # if use_dask_vine:
        #     dask.compute(
        #         graph,
        #         scheduler=scheduler,
        ...
        #         dask.compute(graph, scheduler=client)
        # else:
        #     dask.compute(graph)
        # if print_time: print(f"Write and Merge: {time.time()-t2:.2f}")
        ...
        # Merge partitions
        t4 = time.time()
        merge = self.merge_all_partitions(filesets, partition_merge_size=partition_merge_size)
        ...
        if use_dask_vine:
            ...
            cluster = LocalCluster(n_workers=8)
            with Client(cluster) as client:
                dask.compute(merge, scheduler=client)
        else:
            dask.compute(merge)
        if print_time: print(f"Merge Partitions: {time.time()-t4:.2f}")
