    ...
    def write_partitions(self, fileset, optimize_graph=False):
        ...
        for dataset_name, info in fileset.items():
            ...
            for i in range(events.npartitions): 
                ...
                #d = events.partitions[i:i+1].to_delayed()
                d = events.partitions[i:i+1]
                #d = dak.to_parquet(d, path, compute=False)
                #print(dir(d))
                #exit(1)
                #print(dir(dask.optimize))
                #print(dask.optimize.__kwdefaults__)
                #d = dask.optimize(d)
